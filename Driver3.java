...
public class Driver {
    ...
    public static void main(String[] args) {
        ...
        while(user_Char != 'x') {
        	...
        	System.out.println("Choose an option(1-6) or (A-F) or press (x) to exit: ");
        	userInput = scnr.nextInt();
        	user_Char = scnr.nextChartAt();
        	return;
        }
        if(user_Char == 'x') {
        	break;
    ...
    public static void printShipList(String listType) {
        ...
        if (listType == "name") {
            ...
            for (int i = 0; i <inService.size(); i++) {
            	System.out.println(inService.get(i));
            }
    ...
    public static void addShip() {
    	Ship new_Ship = new Ship(tName, tBalcony, tOceanView, tSuite, tInterior, tInService);
    	if (shipList.contains(new_Ship)) {
    		System.out.println("Name already exists in list.");
    	}
    	else {
    		shipList.add(new_Ship);
    	}
    ...
    public static void addCruise() {
        ...
        // complete this method
    	Cruise new_Cruise = new Cruise(tCruiseName, tShipName, tDeparture, tDestination, tReturn);
    	if(cruiseList.contains(new_Cruise)) {
    		System.out.println("Cruise already exists in list.");
    	}
    	if(shipList.contains(new_Cruise.getCruiseShipName())) {
    		System.out.println("Ship is already assigned to different cruise.")
    	}
    	else {
    		cruiseList.add(new_Cruise);
    	}
